...
ModifiedKalmanFilterEstimate::ModifiedKalmanFilterEstimate(PinocchioInterface pinocchioInterface, CentroidalModelInfo info,
                                           const PinocchioEndEffectorKinematics& eeKinematics, const std::string ns)
    : ModifiedStateEstimateBase(std::move(pinocchioInterface), std::move(info), eeKinematics, ns),
  ...
  sub_ = ros::NodeHandle(ns).subscribe<nav_msgs::Odometry>("tracking_camera/odom/sample", 10, &ModifiedKalmanFilterEstimate::callback, this);
}

vector_t ModifiedKalmanFilterEstimate::update(const ros::Time& time, const ros::Duration& period) {
  ...
  odom.child_frame_id = ns_ + "_base";
  publishMsgs(odom);

  return rbdState_;
}

void ModifiedKalmanFilterEstimate::updateFromTopic() {
  ...
  if (world2odom_.getRotation() == tf2::Quaternion::getIdentity())  // First received
  {
    ...
    try {
      geometry_msgs::TransformStamped tf_msg = tfBuffer_.lookupTransform(ns_ + "_odom", msg->child_frame_id, msg->header.stamp);
  ...
  try {
    geometry_msgs::TransformStamped tf_msg = tfBuffer_.lookupTransform(ns_ + "_base", msg->child_frame_id, msg->header.stamp);
...
void ModifiedKalmanFilterEstimate::callback(const nav_msgs::Odometry::ConstPtr& msg) {
  buffer_.writeFromNonRT(*msg);
  topicUpdated_ = true;
}

nav_msgs::Odometry ModifiedKalmanFilterEstimate::getOdomMsg() {
...
void ModifiedKalmanFilterEstimate::loadSettings(const std::string& taskFile, bool verbose) {
