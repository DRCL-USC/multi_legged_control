...
namespace legged {
SphereVisualization::SphereVisualization(const std::string ns, PinocchioInterface pinocchioInterface, CentroidalModelInfo centroidalModelInfo,
...
void SphereVisualization::update(const SystemObservation& observation) {
  if (observation.time - lastTime_ > minPublishTimeDifference_) {
    ...
    for (int i = 0; i < numSpheres.size(); ++i) {
      ...
      marker.id = i;
      marker.header.frame_id = ns_ + "_odom";
