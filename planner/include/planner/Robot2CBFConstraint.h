...
namespace ocs2
{
  namespace planner
  {
    ...
    class Robot2CBFConstraint final : public StateConstraint
    {
    public:
      Robot2CBFConstraint(std::shared_ptr<Obstacles> obstacles, scalar_array_t radius_array)
          : StateConstraint(ConstraintOrder::Quadratic), radius_array_(radius_array), obstacles_(obstacles) {};

      ~Robot2CBFConstraint() override = default;
      Robot2CBFConstraint *clone() const override { return new Robot2CBFConstraint(*this); }
      ...
      vector_t getValue(scalar_t time, const vector_t &state, const PreComputation &preComp) const override
      {
        ...
        for (size_t i = 0; i < pos_array_.size(); ++i)
        {
          scalar_t B = -(radius_array_[i] + 0.2) * (radius_array_[i] + 0.2) +
                       ...
                       (state(1) + 0.5 - pos_array_[i](1)) * (state(1) + 0.5 - pos_array_[i](1));
          constraint(i) = B +
    ...
    private:
      Robot2CBFConstraint(const Robot2CBFConstraint &other) = default;
